...
public static class VentUsePatch
{
    public static bool Prefix(Vent __instance)
    {
        ...
        if (__instance.name.StartsWith("JackInTheBoxVent_"))
        {
            ...
            MessageWriter writer = AmongUsClient.Instance.StartRpc(CachedPlayer.LocalPlayer.PlayerControl.NetId, (byte)CustomRPC.UseUncheckedVent, SendOption.Reliable);
...
class EmergencyMinigameUpdatePatch
{
    static void Postfix(EmergencyMinigame __instance)
    {
        ...
        {
            roleCanCallEmergency = false;
            statusText = "lawyerCallMeeting".Translate();
            if (Lawyer.isProsecutor) statusText = "prosecutorCallMeeting".Translate();
        }
        ...
        if (__instance.state == 1)
        {
            ...
            __instance.NumberText.text = string.Format("shipMeetingCount".Translate(), localRemaining.ToString(), teamRemaining.ToString());
...
class SurveillanceMinigamePatch
{
    ...
    private static void nightVisionUpdate(SurveillanceMinigame SkeldCamsMinigame = null, PlanetSurveillanceMinigame SwitchCamsMinigame = null, FungleSurveillanceMinigame FungleCamMinigame = null)
    {
        ...
        bool ignoreNightVision = (CustomOptionHolder.camsNoNightVisionIfImpVision.getBool() && hasImpVision(GameData.Instance.GetPlayerById(CachedPlayer.LocalPlayer.PlayerId))) || CachedPlayer.LocalPlayer.Data.IsDead;
