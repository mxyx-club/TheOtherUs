...
class GameModeMenuPatch
{
    ...
    public static bool Prefix(GameModeMenu __instance)
    {
        ...
        float num = ((Mathf.CeilToInt(4f / 10f) / 2f) - 0.5f) * -2.5f;   // 4 for 4 buttons!
        ...
        for (int i = 0; i <= 5; i++)
        {
            ...
            if (entry != GameModes.None)
            {
                ...
                chatLanguageButton.transform.localPosition = new Vector3(num + (num2 / 10 * 2.5f), 2f - (num2 % 10 * 0.5f), 0f);
                if (i <= 2)
