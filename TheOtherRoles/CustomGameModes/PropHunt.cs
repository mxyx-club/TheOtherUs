    ...
    class PropHunt
    {
        ...
        public static void update()
        {
            if (!isPropHuntGM)
            {
                ...
                if (GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.HideNSeek) HudManager.Instance.DangerMeter?.gameObject.SetActive(false);
                return;
            }
        ...
        {  // A bit of a hacky way to make sure that props as well as propable objects are not visible in the dark, while keeping collisions enabled.
            ...
            foreach (Collider2D collider in Physics2D.OverlapCircleAll(PlayerControl.LocalPlayer.transform.position, 500))
            {
                ...
                if (collider.GetComponent<Console>() != null || whiteListed)
                {
                    if (whiteListed)
                    {
                        var newgo = UnityEngine.Object.Instantiate(collider.gameObject, collider.transform.parent);
                        newgo.name = "DONTUSE";
        ...
        public static void IntroCutsceneDestroyPatch(IntroCutscene __instance)
        {
            ...
            MessageWriter writer2 = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.PlayerControl.NetId, (byte)CustomRPC.PropHuntStartTimer, SendOption.Reliable, -1);
            writer2.Write(true);
            ...
            FastDestroyableSingleton<HudManager>.Instance.StartCoroutine(Effects.Lerp(initialBlackoutTime, new Action<float>((p) =>
            {
                ...
                {
                    // start timer
                    MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.PlayerControl.NetId, (byte)CustomRPC.PropHuntStartTimer, SendOption.Reliable, -1);
                    writer.Write(false);
        ...
        public static void PlayerPhysicsAnimationPatch(PlayerPhysics __instance)
        {
            ...
            try
            {
                ...
                if (__instance.myPlayer.Data.IsDead)
                {
                    ...
                    UnityEngine.Object.Destroy(__instance.GetComponent<SpriteRenderer>());
                }
            }
            catch { }
        }
        ...
        {
            if (!isPropHuntGM || target != CachedPlayer.LocalPlayer.PlayerControl) return;
            try
            {
